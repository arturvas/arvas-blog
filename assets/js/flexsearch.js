...
(function () {
  ...
  function rankDocument(doc, query) {
    ...
    const excerpt = normalizeText(doc.display || doc.content);
    const crumb = normalizeText(doc.crumb);
    let score = 0;

    if (title === query) score += 800;
    if (title.startsWith(query)) score += 500;
    if (title.includes(query)) score += 250;
    if (excerpt.startsWith(query)) score += 120;
    if (excerpt.includes(query)) score += 80;
    if (crumb.includes(query)) score += 40;
    if (doc.display !== undefined) score += 30;
  ...
  function collectDocuments(query, limit) {
    const directResults = window.sectionIndex.search(query, limit, {
      enrich: true,
  ...
  async function preloadIndex() {
    ...
    for (const route in data) {
      let pageContent = '';
      ++pageId;
      const crumb = getCrumb(route, data);
      ...
      for (const heading in data[route].data) {
        ...
        sectionIndex.add(titleDoc);
        window.searchDocuments.push(titleDoc);
        ...
        for (let i = 0; i < paragraphs.length; i++) {
          ...
          sectionIndex.add(paragraphDoc);
          window.searchDocuments.push(paragraphDoc);
        }
  ...
  async function search(e) {
    ...
    // Ignore stale searches if the user changed the input while the index loaded.
    ...
    const documents = collectDocuments(normalizedQuery, 12)
  ...
  function displayResults(results, query) {
    ...
    function highlightMatches(text, currentQuery) {
      ...
      const li = createElement(`
        <li>
          <a data-index="${i}" href="${result.route}" class=${i === 0 ? 'hextra-search-active' : ''}>
            ...
            ${result.children.content ? `<div class="hextra-search-excerpt">${highlightMatches(result.children.content, query)}</div>` : ''}
          </a>
        </li>`);
